...
from infrastructure.programversion import PROGRAM_VERSION as PROGRAM_VERSION_FROM_SOURCE
from infrastructure.versionInfo import (PROGRAM_VERSION, getGitBuildIdentification,
                                        getVersionInformationText,
...
def test_get_git_build_identification_returns_description():
    ...
    with patch("infrastructure.versionInfo.subprocess.run",
...
def test_get_git_build_identification_uses_frozen_build_information():
    ...
    with patch("infrastructure.versionInfo.FROZEN_GIT_BUILD_IDENTIFICATION",
               gitBuildIdentification):
        with patch("infrastructure.versionInfo.subprocess.run") as gitDescribe:
...
def test_get_version_information_text():
    gitBuildIdentification = "cewe2pdf-v1.0-build-178-0-g1234567"
    with patch("infrastructure.versionInfo.getGitBuildIdentification",
               return_value=gitBuildIdentification):
...
def test_log_version_information_with_git_build_identification():
    ...
    with patch("infrastructure.versionInfo.getGitBuildIdentification",
               return_value=gitBuildIdentification):
        with patch("infrastructure.versionInfo.mustsee.info") as logInfo:
            logVersionInformation()
